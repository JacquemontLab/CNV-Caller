...
#              intersects it with multiple algorithm-generated TSV files to
...
if [ "$#" -ne 3 ]; then
    ...
    echo "  <algorithms_to_overlap> : A comma-separated list of algorithms with corresponding CNV files."
    echo "                            Each entry should be in the format:"
    echo "                            first_algorithm_name:file,second_algorithm_name:file"
    echo "                            Each file must contain at least the following tab-delimited columns:"
    echo "                            SampleID   Chr   Start   End"
...
# Loop through the algorithms and their corresponding TSV files
for item in ${algorithms_to_overlap//,/ }; do
    # Extract algorithm name (before the colon)
    algo=${item%%:*}
    # Extract TSV file name (after the colon)
    tsvfile=${item##*:}

    # Process the TSV file for the current algorithm:
