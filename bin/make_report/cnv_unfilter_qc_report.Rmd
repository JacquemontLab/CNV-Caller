...
params:
  dataset_name: "unknow"
  dataset: "annotated_child_cnv_penncnv.tsv"  # change this to your dataset ID or pass it when knitting
...
including Mendelian precision estimates at two reciprocal overlap thresholds (0.1 and 0.5),  
CNV length distributions, and copy number state summaries.

Input file should be a TSV with columns including: SampleID, Length, Type, Num_Probes, Observed_in_Parent_0.5, and the Confidence.

Rscript -e "rmarkdown::render('cnv_raw_qc_report.Rmd', params=list(dataset_name='SPARK_PennCNV_raw', dataset='/home/flben/links/projects/rrg-jacquese/flben/Mendelian_precision/data/processed/trioinfered_PC_SPARK.tsv', x_var_list='Confidence,Length,Num_Probes'),
...
```{r extract , echo=FALSE, include=FALSE}
# Load your CNV data file here (change path and parameters accordingly)
child_cnv <- read.delim(params$dataset, stringsAsFactors = FALSE, sep = "\t", header = TRUE)
# child_cnv <- read.delim("~/All_user_common_folder/STUDY_DIRECTORY/Mendelian_precision/data/processed/trioinfered_PC_SPARK.tsv", stringsAsFactors = FALSE, sep = "\t", header = TRUE)
...
### **Number of Trios or Children analyzed:** `r scales::comma(length(unique(child_cnv$SampleID)))`

## 1- CNV Length Distribution Across Samples
...
## Distribution of CNVs by Copy Number State

```{r summary_cn, echo=FALSE}
summary_stats <- table(child_cnv$Copy_Number)
print(summary_stats)
...
labels <- c("0–1 kb", "1–10 kb", "10–100 kb", "100 kb–1 Mb", ">1 Mb")
...
## 2- Mendelian precision across CNV length bins: assessing dataset quality (proportion of child CNVs observed in parents with $\geq 50\%$ reciprocal overlap) across multiple metrics.
    ...
    for (type in c("DEL", "DUP")) {
      ...
      fraction_df <- data_set %>%
        ...
        mutate(
          ...
          size_bin = factor(size_bin, levels = c("<1K", "1-2K", "2-3K", "3-5K", ">5K"))
        )
