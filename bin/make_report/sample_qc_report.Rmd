...
fail_counts <- qc_df %>%
  summarise(
    ...
    fail_Call_Rate = scales::comma(sum(!pass_Call_Rate)),
    fail_LRR_SD = scales::comma(sum(!pass_LRR_SD)),
    fail_BAF_DRIFT = scales::comma(sum(!pass_BAF_DRIFT)),
    fail_WF = scales::comma(sum(!pass_WF)),
    pass_all = scales::comma(sum(pass_all))
  )
