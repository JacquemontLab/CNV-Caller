...
Write a PDF automatic report based on a QC per sample TSV file containing columns: Sex, Call_Rate, LRR_SD, BAF_SD, WF 
...
fail_counts <- qc_df %>%
  summarise(
    total_samples = scales::comma(n()),
    fail_Call_Rate = scales::comma(sum(!pass_Call_Rate,na.rm=TRUE)),
    fail_LRR_SD = scales::comma(sum(!pass_LRR_SD,na.rm=TRUE)),
    fail_BAF_DRIFT = scales::comma(sum(!pass_BAF_DRIFT,na.rm=TRUE)),
    fail_WF = scales::comma(sum(!pass_WF,na.rm=TRUE)),
    pass_all = scales::comma(sum(pass_all,na.rm=TRUE))
  )

# Count samples by Sex, handling missing column
if("Sex" %in% colnames(qc_df)) {
  sex_summary <- qc_df %>%
    count(Sex) %>%
    rename(Sex = Sex, Count = n) %>%
    mutate(Count = scales::comma(Count))
} else {
  sex_summary <- tibble(Sex = "Unknown", Count = scales::comma(nrow(qc_df)))
}
...
ggplot(qc_df, aes(y = Call_Rate, x = "")) +
  ...
  annotate("text", x = 1, y = 0.99, label = "Recommended Threshold: >= 0.98", color = "#E31A1C", size = 3, hjust = -0.0) +
  theme(
    axis.text = element_text(size = 14),
    axis.title = element_text(size = 16)
  )

cat(sprintf("Number of samples NOT passing Call Rate threshold (>= 0.98): %s\n", fail_counts$fail_Call_Rate))
...
cat(sprintf("Number of samples NOT passing LRR_SD threshold (< 0.3): %s\n", fail_counts$fail_LRR_SD))
...
cat(sprintf("Number of samples NOT passing BAF_DRIFT threshold (< 0.01): %s\n", fail_counts$fail_BAF_DRIFT))
...
ggplot(qc_df, aes(y = WF, x = "")) +
  ...
  geom_hline(yintercept = -0.05, color = "#E31A1C", linetype = "dashed", size = 1) +
  annotate("text", x = 1, y = 0.07, label = "Recommended Threshold: < 0.05", color = "#E31A1C", size = 3, hjust = -0.08) +
  annotate("text", x = 1, y = -0.07, label = "Recommended Threshold: > -0.05", color = "#E31A1C", size = 3, hjust = -0.06) +
  theme(
    axis.text = element_text(size = 14),
    axis.title = element_text(size = 16)
  )

cat(sprintf("Number of samples NOT passing WF threshold (between -0.05 and 0.05): %s\n", fail_counts$fail_WF))
